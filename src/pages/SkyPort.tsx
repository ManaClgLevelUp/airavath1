...
import { Landmark, Eye, Crown, Wind, Sun, Radar, ArrowLeft, ChevronRight, Play, CheckCircle2 } from "lucide-react";
    ...
    <div className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative min-h-[85vh] flex items-center overflow-hidden">
        <div className="absolute inset-0">
          <img src={heroImg} alt="AIRAVATH Sky Port rooftop" className="w-full h-full object-cover scale-105" />
        ...
        <div className="relative z-10 container-airavath pt-32 pb-20">
          ...
          <ScrollReveal>
            <span className="font-heading text-[12px] tracking-[6px] text-primary uppercase mb-4 block">Rooftop Landing Infrastructure</span>
            <h1 className="font-heading text-[48px] md:text-[72px] font-semibold text-foreground tracking-futuristic mb-6 leading-[1.1]">Sky Port</h1>
          </ScrollReveal>
            ...
            {[gallery1, gallery2].map((img, i) => (
              <ScrollReveal key={i} delay={0.15 * i}>
                <motion.div className="relative rounded-[12px] overflow-hidden h-[420px] group cursor-pointer" initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }} transition={{ duration: 0.8, delay: 0.15 * i }} viewport={{ once: true }}>
                  <img src={img} alt={`Sky Port concept video ${i + 1}`} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-105" />
                  ...
                  <div className="absolute bottom-4 left-4">
                    <p className="font-body text-[13px] text-foreground/70">{i === 0 ? "Premium Rooftop Lounge Tour" : "eVTOL Rooftop Landing Sequence"}</p>
                  </div>
                </motion.div>
              </ScrollReveal>
            ))}
